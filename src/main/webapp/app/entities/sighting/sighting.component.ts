...
export class SightingComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.sightingService
      ...
      .subscribe(
        (res: HttpResponse<ISighting[]>) => this.paginateSightings(res.body, res.headers),
  ...
  trackId(index: number, item: ISighting) {
    return item.id;
  }
  ...
  registerChangeInSightings() {
    this.eventSubscriber = this.eventManager.subscribe('sightingListModification', response => this.loadAll());
  }
  ...
  protected paginateSightings(data: ISighting[], headers: HttpHeaders) {
